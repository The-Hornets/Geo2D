module Geo2d
  class Segment
    ...
    def initialize(start_point,end_point)
      raise ArgumentError, "Both arguments must be Point objects" unless start_point.is_a?(Point) && end_point.is_a?(Point)
    ...
    def contains_point?(point)
      ...
      return (point.x >= min_x - EPSILON) && (point.y >= min_y - EPSILON) && (point.x <= max_x + EPSILON) && (point.y <= max_y + EPSILON)
    end
